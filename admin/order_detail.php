          ...
          <div class="col-md-12">
            <div class="card">
              ...
              <?php
                ...
                $stmt = $pdo->prepare("SELECT * FROM sale_order_details WHERE sale_orders_id=".$_GET['id']);
                $stmt->execute();
                $rawResult = $stmt->fetchAll();
                $total_page = ceil(count($rawResult) / $numOfrecs);

                $stmt = $pdo->prepare("SELECT * FROM sale_order_details WHERE sale_orders_id=".$_GET['id']." LIMIT $offset,$numOfrecs");
                $stmt->execute();
                  ...
                  <tbody>
                    ...
                    ?>
                        <tr>
                          ...
                          <td><?php echo escape($value['quantity']*$value['price'])?></td>
                ...
                <nav aria-label="Page navigation example" style="float:right">
                  <ul class="pagination">
                    ...
                    <li class="page-item <?php if($pageno>=$total_page){echo 'disabled';}?>">
                      <a class="page-link" href="<?php if($pageno>=$total_page){echo '#';}else{echo '?id='.$_GET['id'].'&pageno='.($pageno+1);}?>">Next</a>
                    </li>
                    <li class="page-item"><a class="page-link" href="?id=<?php echo $_GET['id']?>&pageno=<?php echo $total_page;?>">Last</a></li>
                  </ul>
                </nav>
              </div>
              <!-- /.card-body -->
